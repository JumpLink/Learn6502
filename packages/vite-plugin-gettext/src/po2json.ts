...
async function createDefaultLanguageJson(
  ...
  await fs.writeFile(
    ...
    JSON.stringify(defaultLanguageJson, null, 2)
  );
}
  ...
  async function convertPoToJson() {
    try {
      ...
      for (const lang of nonDefaultLanguages) {
        ...
        await fs.writeFile(
          jsonFile,
          JSON.stringify(simplifiedTranslations, null, 2)
        );
      }

      // Create the default language file (with all original strings as both keys and values)
      await createDefaultLanguageJson(jsonDirectory, allTranslations, defaultLanguage, verbose, pluginName);
    } catch (error) {
