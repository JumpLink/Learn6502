...
- On reset (simulator): `P = $30` (bit5 and `B` set; `D` and `I` clear)
...
- **Indexed Indirect** (`($c0,X)`): Add X to zero page address, then use result as pointer to get actual target address
- **Indirect Indexed** (`($c0),Y`): Use zero page address as pointer to get base address, then add Y to that base
- **Relative** (`label`): Branch uses signed 8‑bit offset from the next instruction (−128…+127)
...
- Also useful: **BIT**, **CMP/CPX/CPY**, **PHP/PLP**, **TSX/TXS**, **SED/CLD**, **SEI/CLI**, **BMI/BPL**, **BVS/BVC**

## Quick tips

- **SBC needs carry set**: Set `C` before subtracting (works as inverse borrow). `C` clear after SBC indicates a borrow occurred.
- **Compare ops**: `CMP/CPX/CPY` set `N/Z/C` like a subtraction (register − memory) without storing a result.
